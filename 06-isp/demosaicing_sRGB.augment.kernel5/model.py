...
class Model():
    ...
    def build(self):
        ...
        for i in range(self.depth - 2):
            x = self._conv_layer(name = 'conv'+ str(i+1), inp = x, kernel_shape = [5, 5, 64, 64 ], stride = 1, is_training = is_training )


        x = self._conv_layer(name='conv' + str(self.depth), inp = x, kernel_shape=[5, 5, 64, config.nr_channel],
