...
class Model():
    ...
    def build(self):
        ...
        x = self._conv_layer(name='conv0', inp=data,
                             kernel_shape=[7, 7, config.nr_channel, 64], stride=1,
                             is_training=is_training) # Nx32x32x32


        for i in range(self.depth - 2):
            x = self._conv_layer(name = 'conv'+ str(i+1), inp = x, kernel_shape = [7, 7, 64, 64 ], stride = 1, is_training = is_training )


        x = self._conv_layer(name='conv' + str(self.depth), inp = x, kernel_shape=[7, 7, 64, config.nr_channel],
